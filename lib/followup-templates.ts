...
function buildStage1(name: string, situation: Situation): string {
  ...
  const nudge: Record<Situation, string> = {
    ...
    resigned: 'גם מי שהתפטר לרוב זכאי לזכויות — חבל לפספס. מתי נוח לך ל-10 דקות?',
...
function buildStage2(name: string): string {
  return [
    ...
    `בבדיקות שאני עושה, ברוב המקרים מתגלות זכויות שאנשים פשוט לא ידעו שמגיעות להם 💡`,
    `בדיקה ראשונית של התיק שלך — *ללא עלות*.`,
    `שווה 10 דקות. מתי נוח לך?`,
  ].join('\n')
}
